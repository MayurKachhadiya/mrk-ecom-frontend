...
const ProductDetails = () => {
  ...
  const getProductDetails = async (pageNo = 1) => {
    ...
    try {
      const res = await axios.post(
        `https://mrk-ecom-backend.onrender.com/product/details/${pid}?page=${pageNo}&limit=10`,
  ...
  const AddToCart = (newQty) => {
    ...
    axios
      .post(
        "https://mrk-ecom-backend.onrender.com/cart/add",
        {
          UserId: resUserData.id,
          ProductId: pid,
          quantity: newQty,
  ...
  const handleDeleteConfirm = () => {
    ...
    axios
      .delete(
        `https://mrk-ecom-backend.onrender.com/review/delete/${deletedReviewId}`,
        {
          headers: {
            Authorization: `Bearer ${BEARER_TOKEN}`,
          },
      ...
      {loading ? (
        ...
        <>
          <AddReview
            ...
            closeDialog={() => setReviewDialog(false)}
            handleEdit={isEdit}
            onSuccess={() => {
              setPage(1);
              setProductReviews([]);
